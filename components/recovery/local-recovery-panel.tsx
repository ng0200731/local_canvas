...
function ArchiveCounts({ archive }: { archive: LocalRecoveryArchive }) {
  ...
  return (
    <div className="bg-border grid grid-cols-2 gap-px overflow-hidden rounded-xl border sm:grid-cols-5">
...
export function LocalRecoveryPanel() {
  ...
  useEffect(() => {
    void createLocalRecoveryArchive()
      .then(setArchive)
      .catch(() => toast.error("Could not read local records."));
  }, []);
  ...
  async function chooseFile(file: File): Promise<void> {
    try {
      setPending(localRecoveryArchiveSchema.parse(JSON.parse(await file.text()) as unknown));
    } catch {
      setPending(null);
      toast.error("This is not a valid Infinite Canvas backup.");
    }
  }
  ...
  async function uploadToSupabase(): Promise<void> {
    ...
    try {
      ...
      toast.success(
        `Imported ${result.projects} projects, ${result.canvases} canvases, and ${result.nodes} nodes.`,
      );
    } catch (error) {
  ...
  return (
    <main className="text-foreground min-h-screen bg-[radial-gradient(circle_at_top_left,var(--muted),transparent_42%)] px-5 py-12 sm:px-10">
      <section className="mx-auto max-w-5xl">
        <div className="mb-10 flex items-start justify-between gap-6 border-b pb-8">
          <div>
            <p className="text-muted-foreground mb-3 font-mono text-xs tracking-[0.25em] uppercase">
              Local archive{archive ? ` · ${archive.origin}` : ""}
            </p>
            <h1 className="max-w-3xl text-4xl font-semibold tracking-tight sm:text-6xl">
              Recover before you migrate.
            </h1>
            <p className="text-muted-foreground mt-4 max-w-2xl">
              Export this port, then import its file on the other port. Nothing is deleted, and a
              backup is downloaded before every merge.
            </p>
          </div>
        ...
        {archive ? (
          <ArchiveCounts archive={archive} />
        ) : (
          <div className="bg-muted h-24 animate-pulse rounded-xl" />
        )}
        <div className="mt-8 grid gap-5 md:grid-cols-2">
          <article className="bg-background/90 rounded-2xl border p-6 shadow-sm">
            <Download className="mb-8 size-7" />
            <h2 className="text-xl font-semibold">1. Export this port</h2>
            <p className="text-muted-foreground mt-2 min-h-12 text-sm">
              Includes canvas graphs stored in IndexedDB, not only visible project metadata.
            </p>
            <Button className="mt-7 w-full" onClick={() => void exportCurrent()}>
              Download backup
            </Button>
          </article>
          <article className="bg-background/90 rounded-2xl border p-6 shadow-sm">
            <Upload className="mb-8 size-7" />
            <h2 className="text-xl font-semibold">2. Merge another port</h2>
            <p className="text-muted-foreground mt-2 min-h-12 text-sm">
              Select the backup downloaded from port 3000 or 3001. Matching IDs keep the newest
              version.
            </p>
            <input
              ref={inputRef}
        ...
        <article className="mt-5 rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-6">
          <CloudUpload className="mb-5 size-7" />
          <h2 className="text-xl font-semibold">3. Upload this port to Supabase</h2>
          <p className="text-muted-foreground mt-2 text-sm">
            Sign in first. A JSON backup downloads automatically, then validated records are written
            under your account. Local data is not removed.
          </p>
          <Button
            className="mt-5"
            disabled={uploading || !archive}
        ...
        {pending && (
          <div className="mt-6 rounded-2xl border border-amber-500/40 bg-amber-500/10 p-6">
            <div className="flex gap-3">
              <ShieldCheck className="mt-0.5 size-5 shrink-0" />
              <div className="flex-1">
                <h2 className="font-semibold">Ready to merge backup from {pending.origin}</h2>
                <p className="text-muted-foreground mt-1 text-sm">
                  Review its counts below. A pre-merge backup downloads automatically.
                </p>
                <div className="mt-4">
                  <ArchiveCounts archive={pending} />
                </div>
                <div className="mt-5 flex gap-3">
                  <Button onClick={() => void mergePending()}>Merge records</Button>
